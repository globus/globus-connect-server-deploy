...
## About The Project
This project provides tools that support the deployment of [Globus Connect Server](https://www.globus.org/globus-connect) using Docker. It serves as a starting point for endpoint administrators interested in containerized deployment for production or testing purposes.

## Status and Limitations
The current project status is **reference implementation** meaning it is a working example and starting point, not a fully supported production deployment. It will successfully build and launch a new GCS node, add it to an existing GCS endpoint and remove it from the endpoint when the container exits. Site administrators with Docker expertise will likely need to adapt these files to fit their specific environment and requirements. It has several limitations:
